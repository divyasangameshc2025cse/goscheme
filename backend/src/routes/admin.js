...
const { runScrape } = require('../scraper/scrapeSchemes');

const router = express.Router();

// Simple in-process guard so two scrape requests can't run concurrently
let scrapeInProgress = false;

// POST /api/admin/scrape-schemes
// Triggers the live government-scheme scraper and upserts results into the
// DB, sector-classified. Kicked off manually from the admin dashboard (or a
// scheduled job you run yourself, e.g. via cron / node-cron) rather than on
// every request, since scraping is comparatively slow and rate-limited.
router.post('/scrape-schemes', async (req, res) => {
  if (scrapeInProgress) {
  ...
  try {
    ...
    return res.json({ success: true, message: 'Scrape complete', summary });
  } catch (err) {
    console.error('Scrape trigger error:', err);
    return res.status(500).json({ success: false, message: 'Scrape failed', error: err.message });
  } finally {
